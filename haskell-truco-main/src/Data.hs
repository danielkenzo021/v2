...
compararCartas c1 c2 manilha
    ...
    | otherwise            = EQ
  where
    -- Função para verificar se uma carta é a manilha (considerando apenas o número)
    ehManilha :: Carta -> Numero -> Bool
    ehManilha (Carta num _) manilhaNum = num == manilhaNum

    -- Função para comparar naipes quando ambas as cartas são manilhas
    compareNaipe :: Carta -> Carta -> Ordering
    compareNaipe (Carta _ naipe1) (Carta _ naipe2) = compareNaipeRanking naipe1 naipe2

    -- Função para comparar naipes com a hierarquia específica do truco
